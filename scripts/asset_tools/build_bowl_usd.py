"""Build a self-contained BOWL.usd directly with pxr, mirroring build_cup_usd.py:
/BOWL Xform (double translate/orient/scale) + /BOWL/mesh Mesh (points/faces/extent +
physics convexDecomposition + embedded tet_* attrs) + a bound UsdPreviewSurface so the
in-sim bowl renders lit/coloured (the UIPC loader swaps the mesh for the tet surface but
keeps the material binding).
...
Usage (pxr lives in isaacsim extscache):
  ...
  PYTHONPATH=$EXT LD_LIBRARY_PATH=$EXT/bin:$EXT/pxr/.libs \
    python scripts/asset_tools/build_bowl_usd.py <in.obj> <tet.npz> <out.usd>
"""
...
ROOT = "/BOWL"
COLOR = (0.80, 0.82, 0.86)   # off-white ceramic
...
mesh = UsdGeom.Mesh.Define(stage, f"{ROOT}/mesh")
...
bind_material(stage, ROOT, prim, COLOR)

stage.GetRootLayer().Save()
print(f"[build_bowl_usd] {out_path}: {len(V)} verts / {len(F)} faces / {len(d['tet_indices'])//4} tets, self-contained")
